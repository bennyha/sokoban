...
public class Level implements Serializable {
	...
	public Level() {}
	public Level(ArrayList<ArrayList<Component>> grid) {
		try 
		{			
			this.grid = grid;
	...
	private void findSolCoordinates() {
		for(int i = 0; i < grid.size(); i++) 
			for(int j = 0; j < grid.get(i).size(); j++) {
				Component w = grid.get(i).get(j);
				if (w.getComponentType() == ComponentType.TARGET) { 
					solutionCoordinates.add(w.getPosition()); 
					}
			}
	}
	...
	public Player getPlayer() {
		...
		for (int i = 0; i < grid.size(); i++)
		{
			for (int j = 0; j < grid.get(i).size(); j++)
			{
				if (grid.get(i).get(j).getComponentType() ==ComponentType.PLAYER)
	...
	public final ArrayList<ArrayList<Component>> getGrid() {
		return grid;
		}
	
	public void setGrid(ArrayList<ArrayList<Component>> grid) {
		this.grid = grid;
		}
	...
	public boolean checkIfWin() {
		...
		for(Point p : solutionCoordinates) {
			int x = (int)p.getX();
			int y = (int)p.getY();
			if(grid.get(x).get(y).getComponentType() != ComponentType.BAGGAGE) {
				return false;
			}
		}
		return true;
	...
	public Component getWorldObject(Point position) {
